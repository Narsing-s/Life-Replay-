...
function createStorage() {
  ...
  const keyFor = ({ userId, id, originalName = '' }) => { const ext = path.extname(originalName).toLowerCase().replace(/[^a-z0-9.]/g, ''); return `users/${userId}/${id}${ext}`; };
  async function putFile({ userId, id, filePath, originalName, mimeType, checksum }) { const key = keyFor({ userId, id, originalName }); if (!remote) { const target = path.join(localRoot, path.basename(key)); fs.copyFileSync(filePath, target); return { key: target, url: `/media/${path.basename(target)}` }; } const body = fs.createReadStream(filePath); await client.send(new PutObjectCommand({ Bucket: cfg.objectStorage.bucket, Key: key, Body: body, ContentType: mimeType, Metadata: { checksum: checksum || '' }, ServerSideEncryption: cfg.objectStorage.endpoint.includes('amazonaws.com') ? 'AES256' : undefined })); return { key, url: null }; }
  async function putBuffer({ userId, id, buffer, originalName, mimeType, checksum }) { const tmp = path.join(localRoot, `.upload-${crypto.randomUUID()}`); fs.writeFileSync(tmp, buffer); try { return await putFile({ userId, id, filePath: tmp, originalName, mimeType, checksum }); } finally { try { fs.unlinkSync(tmp); } catch {} } }
  async function signedReadUrl(key, expiresIn = 300) { if (!remote) return `/api/v1/media/file/${encodeURIComponent(path.basename(key))}`; return getSignedUrl(client, new GetObjectCommand({ Bucket: cfg.objectStorage.bucket, Key: key }), { expiresIn }); }
