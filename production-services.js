...
async function askLLM({ question, memories }) {
  ...
  const safeContext = memories.slice(0, 50).map(m => ({
    id: m.id, title: m.title, caption: m.caption, place: m.place, date: m.date,
    summary: m.summary || '', category: m.category || '', mood: m.mood || ''
  }));
  const response = await fetch(`${base}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      temperature: 0.2,
      messages: [
        { role: 'system', content: 'You are Life Replay, a private memory assistant. Answer only from the supplied authenticated user memory context. Never invent memories, people, dates, places, or facts. If the context does not answer the question, say so.' },
        { role: 'user', content: JSON.stringify({ question, memories: safeContext }) }
      ]
    })
  });
...
function providerStatus() {
  return {
    ...
    objectStorage: {
      provider: env('S3_ENDPOINT') || env('R2_ENDPOINT') ? 's3-compatible' : 'filesystem',
      configured: Boolean(env('S3_ENDPOINT') || env('R2_ENDPOINT'))
    },
    ...
    observability: { provider: env('OTEL_EXPORTER_OTLP_ENDPOINT') ? 'opentelemetry' : 'structured-logs', configured: configured('OTEL_EXPORTER_OTLP_ENDPOINT') },
    multiInstance: { ready: Boolean(env('DATABASE_URL') && (env('S3_ENDPOINT') || env('R2_ENDPOINT')) && env('REDIS_URL')) }
  };
}

module.exports = { env, configured, sendEmail, askLLM, embed, randomToken, sha256, providerStatus };
